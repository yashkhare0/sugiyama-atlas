...
import { buildJourneyGraph } from "./build-graph.js";
import { detectFramework, discoverNextRoutes, discoverViteRoutes } from "./framework-adapters.js";
...
function expressionInitializer(expression) {
...
function expressionDeclarations(node) {
...
function objectPropertyInitializer(expression, propertyName, seen) {
    ...
    if (Node.isPropertyAccessExpression(expression)) {
        const parentInitializer = objectPropertyInitializer(expression.getExpression(), expression.getName(), new Set(seen));
        if (parentInitializer)
            return objectPropertyInitializer(parentInitializer, propertyName, new Set(seen));
    }
...
function staticExpressionValue(expression, seen = new Set(), bindings = new Map()) {
    if (!expression)
        return undefined;
    if (Node.isIdentifier(expression) && bindings.has(expression.getText()))
        return bindings.get(expression.getText());
    ...
    if (Node.isConditionalExpression(expression)) {
        ...
        if (whenTrue !== undefined && whenTrue === whenFalse)
            return whenTrue;
    }
    ...
    if (Node.isPropertyAccessExpression(expression)) {
        ...
        while (Node.isPropertyAccessExpression(rootExpression))
            rootExpression = rootExpression.getExpression();
...
function staticResolvedAttributeValue(element, name) {
    ...
    if (direct !== undefined || !expression)
        return direct;
    const callSiteValues = new Set(componentPropExpressions(expression)
        .map((value) => staticExpressionValue(value))
        .filter((value) => value !== undefined));
    return callSiteValues.size === 1 ? [...callSiteValues][0] : undefined;
}
function staticExpressionCandidates(expression, seen = new Set()) {
    if (!expression)
        return [];
    const key = `${expression.getSourceFile().getFilePath()}:${expression.getStart()}`;
    if (seen.has(key))
...
function attributeDestinationCandidates(element) {
            ...
            if (Node.isJsxExpression(child)) {
                ...
                if (expression && (Node.isStringLiteral(expression) || Node.isNoSubstitutionTemplateLiteral(expression))) {
                    ...
                    if (alternatives.length > 0)
                        values.push([...new Set(alternatives)].join(" / "));
                }
            }
        }
    };
...
function associatedLabel(node, opening) {
    const parentLabel = node.getAncestors().find((ancestor) => Node.isJsxElement(ancestor) && tagName(ancestor.getOpeningElement()) === "label");
    const parentText = parentLabel && staticText(parentLabel);
    if (parentText)
        return parentText;
    const tooltip = node.getAncestors().find((ancestor) => Node.isJsxElement(ancestor) && /Tooltip$/.test(tagName(ancestor.getOpeningElement())));
    if (tooltip) {
        const tooltipOpening = tooltip.getOpeningElement();
        const tooltipLabel = staticResolvedAttributeValue(tooltipOpening, "label") ??
    ...
    if (idValue) {
        for (const label of node.getSourceFile().getDescendantsOfKind(SyntaxKind.JsxElement)) {
            if (tagName(label.getOpeningElement()) !== "label")
                continue;
            if (staticAttributeValue(label.getOpeningElement(), "htmlFor") !== idValue)
                continue;
    ...
    if (name)
        return humanizeDynamicLabel(name);
    let container = node.getParent();
    for (let depth = 0; container && depth < 3; depth += 1, container = container.getParent()) {
        if (!Node.isJsxElement(container))
            continue;
...
function dynamicText(element) {
    const semanticExpression = (expression) => {
        ...
        if (Node.isParenthesizedExpression(expression))
            return semanticExpression(expression.getExpression());
...
function humanizeDynamicLabel(value) {
    if (!value || !/^[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*$/.test(value))
        return value;
    const parts = value.split(".");
    if (parts.at(-1)?.toLowerCase() === "url")
        parts.pop();
    const label = parts
...
function isSourceLabelExpression(value) {
    return Boolean(value && /^[A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*$/.test(value) && humanizeDynamicLabel(value));
}
function defaultValueForIdentifier(node, identifier) {
...
function handlerFrom(sourceExpression) {
    if (!sourceExpression)
        return undefined;
...
function componentOpensNewTab(element, seen = new Set()) {
    ...
    return symbol.getDeclarations().some((declaration) => {
        ...
        return declaration
            .getDescendants()
            .filter((node) => Node.isJsxOpeningElement(node) || Node.isJsxSelfClosingElement(node))
            .some((node) => (tagName(node) === "a" && staticAttributeValue(node, "target") === "_blank") ||
            (/^[A-Z]/.test(tagName(node)) && componentOpensNewTab(node, seen)));
    });
}
function kindFor(tag, href, events, target) {
    if (href)
        return target === "_blank" || /^(https?:|mailto:)/.test(href) ? "external-link" : "link";
    if (tag === "form" || events.includes("onSubmit"))
        return "form";
...
function expressionFromAttribute(property) {
    const initializer = property.getInitializer();
    if (!initializer)
        return undefined;
    if (Node.isJsxExpression(initializer))
...
function componentPropExpressions(expression) {
    const parameter = parameterProp(expression);
    if (!parameter || !componentCallableName(parameter.callable))
        return [];
    return componentPropIndex(expression.getProject()).get(`${callableKey(parameter.callable)}:${parameter.prop}`) ?? [];
}
function resolveHandlerExpression(expression, seen = new Set()) {
    const key = `${expression.getSourceFile().getFilePath()}:${expression.getStart()}`;
    if (seen.has(key))
        return undefined;
    seen.add(key);
    if (Node.isArrowFunction(expression) || Node.isFunctionExpression(expression)) {
        ...
        for (const call of expression.getDescendantsOfKind(SyntaxKind.CallExpression)) {
            ...
            if (!Node.isIdentifier(callee) && !Node.isPropertyAccessExpression(callee))
                continue;
            if (/^(?:set[A-Z]|router\.|toast\.|Math\.|Object\.|Array\.)/.test(callee.getText()))
                continue;
            const resolved = resolveHandlerExpression(callee, new Set(seen));
            if (resolved && resolved !== callee.getText())
                parts.push(resolved);
        }
...
function eventHandlerExpression(element, event) {
    const initializer = attribute(element, event)?.getInitializer();
    if (!initializer || !Node.isJsxExpression(initializer))
        return attributeValue(element, event);
    const expression = initializer.getExpression();
    if (!expression)
        return undefined;
    return resolveHandlerExpression(expression) ?? expression.getText();
}
function destinationFrom(element, events, sourceExpression) {
...
const stateOwnershipCache = new WeakMap();
function stateOwnershipIndex(sourceFile) {
    const cached = stateOwnershipCache.get(sourceFile);
    if (cached)
        return cached;
    const index = { ownersByState: new Map(), statesBySetter: new Map() };
    for (const declaration of sourceFile.getDescendantsOfKind(SyntaxKind.VariableDeclaration)) {
        const name = declaration.getNameNode();
        if (!Node.isArrayBindingPattern(name) || name.getElements().length < 2)
            continue;
        ...
        if (!stateName || !setterName)
            continue;
        const states = index.statesBySetter.get(setterName) ?? new Set();
    ...
    for (const opening of [
        ...
        for (const stateName of new Set([...index.statesBySetter.values()].flatMap((states) => [...states]))) {
            if (!new RegExp(`\\b${stateName}\\b`).test(openValue))
                continue;
            const owner = tagName(opening).replace(/(?:Root|Content)$/, "");
            if (!/^(?:AlertDialog|Dialog|Drawer|DropdownMenu|Popover|Sheet)$/.test(owner))
                continue;
            const owners = index.ownersByState.get(stateName) ?? new Set();
            owners.add(owner);
...
function controlledStateOutcome(node, sourceExpression) {
    const index = stateOwnershipIndex(node.getSourceFile());
    const visibleOutcomes = [];
    const setterMatches = [...sourceExpression.matchAll(/\b(set[A-Z][A-Za-z0-9_$]*)\s*\(\s*([^)]*)/g)].map((match) => ({ argument: match[2], setterName: match[1] }));
    ...
    for (const { argument, setterName } of setterMatches) {
        const stateNames = index.statesBySetter.get(setterName) ?? new Set();
        ...
        const controlsRenderedJsx = node
            ...
            .some((identifier) => identifier.getText() === stateName &&
            Boolean(identifier.getFirstAncestorByKind(SyntaxKind.JsxExpression)));
        if (!controlsRenderedJsx)
            continue;
        const subject = stateName
            ...
            .toLowerCase()
            .replace(/^./, (letter) => letter.toUpperCase());
        visibleOutcomes.push(/^(?:is|has|show)[A-Z]/.test(stateName)
...
function resolvedDeclarations(node) {
    const symbol = node.getSymbol();
    if (!symbol)
        return [];
    return [...symbol.getDeclarations(), ...(symbol.getAliasedSymbol()?.getDeclarations() ?? [])];
}
function belongsToRenderScope(node, scope) {
    for (const ancestor of node.getAncestors()) {
        if (ancestor === scope)
            return true;
        if (Node.isFunctionDeclaration(ancestor))
        ...
        if (Node.isArrowFunction(ancestor) || Node.isFunctionExpression(ancestor)) {
            if (ancestor.getFirstAncestorByKind(SyntaxKind.JsxAttribute))
                return false;
            const jsxExpression = ancestor.getParentIfKind(SyntaxKind.JsxExpression);
            if (jsxExpression?.getParentIfKind(SyntaxKind.JsxElement))
                continue;
            const call = ancestor.getParentIfKind(SyntaxKind.CallExpression);
            const callee = call?.getExpression();
            if (!callee || !Node.isPropertyAccessExpression(callee) || callee.getName() !== "map")
                return false;
        }
    }
    return false;
}
function defaultRenderScope(entry) {
...
function renderedJsx(entry, sourceRoot) {
    ...
    const withinSource = (node) => path.resolve(node.getSourceFile().getFilePath()).startsWith(`${path.resolve(sourceRoot)}${path.sep}`);
    while (pending.length > 0) {
        const scope = pending.pop();
        if (!scope)
            continue;
        const scopeKey = `${scope.getSourceFile().getFilePath()}:${scope.getStart()}`;
        if (visited.has(scopeKey))
            continue;
    ...
    return [...result.values()].sort((left, right) => left.getSourceFile().getFilePath().localeCompare(right.getSourceFile().getFilePath()) || left.getStart() - right.getStart());
}
function redirectCandidates(expression, seen = new Set()) {
    if (!expression)
        return [];
    const key = `${expression.getSourceFile().getFilePath()}:${expression.getStart()}`;
    if (seen.has(key))
...
function staticRedirects(entry) {
    const scope = defaultRenderScope(entry);
    if (!scope)
        return [];
    const redirects = new Set();
    for (const call of scope.getDescendantsOfKind(SyntaxKind.CallExpression)) {
        if (!belongsToRenderScope(call, scope) || call.getExpression().getText() !== "redirect")
            continue;
        for (const destination of redirectCandidates(call.getArguments()[0]))
            redirects.add(destination);
    }
    return [...redirects].sort();
}
function unresolvedRedirects(entry, sourceRoot) {
    const scope = defaultRenderScope(entry);
    if (!scope)
        return [];
    const unresolved = [];
    for (const call of scope.getDescendantsOfKind(SyntaxKind.CallExpression)) {
        ...
        if (!argument || redirectCandidates(argument).length > 0)
            continue;
...
function compareInteractions(left, right) {
    return (left.evidence.file.localeCompare(right.evidence.file) ||
        left.evidence.line - right.evidence.line ||
        left.evidence.column - right.evidence.column ||
        left.id.localeCompare(right.id));
}
function dynamicLabelExpression(node, opening) {
...
function staticCallSiteLabel(expression) {
    if (!expression)
        return undefined;
    const direct = staticExpressionValue(expression);
    if (direct !== undefined)
        return direct;
    const values = new Set(componentPropExpressions(expression)
        .map((value) => staticExpressionValue(value))
        .filter((value) => Boolean(value)));
    return values.size === 1 ? [...values][0] : undefined;
}
function interactionFrom(node, sourceRoot, routes) {
    const opening = Node.isJsxElement(node) ? node.getOpeningElement() : node;
    if (attribute(opening, "asChild") &&
        Node.isJsxElement(node) &&
        node
            .getDescendants()
            .some((descendant) => (Node.isJsxOpeningElement(descendant) || Node.isJsxSelfClosingElement(descendant)) &&
    ...
    const localComposite = /^[A-Z]/.test(tag) &&
    ...
    if (localComposite && !attribute(opening, "href"))
        return undefined;
    ...
    const explicitLabel = staticAttributeValue(opening, "aria-label") ??
    ...
    const dynamicLabel = attributeValue(opening, "aria-label") ??
    ...
    const callSiteLabel = staticCallSiteLabel(dynamicLabelExpression(node, opening)) ??
        (Node.isJsxElement(node)
            ? staticCallSiteLabel(node
                .getJsxChildren()
                .filter(Node.isJsxExpression)
                .map((child) => child.getExpression())
                .find((expression) => Boolean(expression)))
    ...
    if (!sourceExpression) {
        const trigger = node
            .getAncestors()
            .find((ancestor) => Node.isJsxElement(ancestor) &&
            /(?:Collapsible|Dialog|Popover|DropdownMenu|Sheet|Drawer)Trigger$/.test(tagName(ancestor.getOpeningElement())));
        if (trigger)
            sourceExpression = `onClick=toggle${tagName(trigger.getOpeningElement()).replace(/Trigger$/, "")}()`;
    }
    if (!href &&
        sourceExpression &&
        events.every((event) => /(?:^|=>\s*)(?:event|e)\.(?:stopPropagation|preventDefault)\(\)\s*$/.test(attributeValue(opening, event) ?? ""))) {
        return undefined;
    }
    ...
    const closeOwner = node
        .getAncestors()
        .find((ancestor) => Node.isJsxElement(ancestor) && /(?:Dialog|Drawer|Popover|Sheet)Close$/.test(tagName(ancestor.getOpeningElement())));
    ...
    const formOwner = node.getAncestors().find((ancestor) => Node.isJsxElement(ancestor) && tagName(ancestor.getOpeningElement()) === "form");
...
function mappedCollectionRoot(node) {
    let root;
    for (const ancestor of node.getAncestors()) {
        if (!Node.isArrowFunction(ancestor) && !Node.isFunctionExpression(ancestor))
            continue;
...
function interactionsFrom(node, sourceRoot, routes) {
    ...
    if (!destinationAccess)
        return [interaction];
    const rawDynamicLabel = attributeValue(opening, "aria-label") ??
        attributeValue(opening, "label") ??
        attributeValue(opening, "title") ??
        (Node.isJsxElement(node) ? dynamicText(node) : undefined);
    const dynamicLabel = rawDynamicLabel?.match(/^([A-Za-z_$][\w$]*)\.([A-Za-z_$][\w$]*)$/);
    if (!dynamicLabel || dynamicLabel[1] !== destinationAccess[1])
        return [interaction];
    ...
    for (const objectLiteral of objectLiterals) {
        ...
        if (!destinationProperty || !Node.isPropertyAssignment(destinationProperty))
            continue;
        if (!labelProperty || !Node.isPropertyAssignment(labelProperty))
            continue;
...
function extractFrameworkJourneys(config, cwd, framework) {
    ...
    const routeSources = framework === "next" ? discoverNextRoutes(project, config, cwd) : discoverViteRoutes(project, config, cwd);
    const duplicatePath = routeSources.find((source, index) => routeSources.findIndex((candidate) => candidate.path === source.path) !== index);
    if (duplicatePath)
        throw new Error(`${framework} adapter emitted duplicate route ${duplicatePath.path}`);
    const interactionsById = new Map();
    const routes = [];
    const collectRoute = (entry, routePath, area, kind) => {
        const routeInteractionIds = new Set();
        ...
        routes.push({
            area,
            file: toPosix(path.relative(sourceRoot, entry.getFilePath())),
            interactions: [...routeInteractionIds].sort((leftId, rightId) => compareInteractions(interactionsById.get(leftId), interactionsById.get(rightId))),
            kind,
            path: routePath,
    ...
    for (const source of routeSources)
        collectRoute(source.file, source.path, source.area, source.kind);
    for (const layoutRoute of routes.filter((route) => route.kind === "layout" && route.path.endsWith(" shared layout"))) {
        const layoutPath = layoutRoute.path.replace(/ shared layout$/, "");
        const childPages = routes.filter((route) => route.kind === "page" && (route.path === layoutPath || route.path.startsWith(`${layoutPath}/`)));
        for (const childPage of childPages) {
            childPage.interactions = [...new Set([...childPage.interactions, ...layoutRoute.interactions])].sort((leftId, rightId) => compareInteractions(interactionsById.get(leftId), interactionsById.get(rightId)));
...
export function extractJourneys(config, cwd = process.cwd()) {
    return extractFrameworkJourneys(config, cwd, detectFramework(config, cwd));
}
export function extractNextJourneys(config, cwd = process.cwd()) {
    return extractFrameworkJourneys({ ...config, framework: "next" }, cwd, "next");
}
//# sourceMappingURL=extract-next.js.map