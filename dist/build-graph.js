...
function routeMatches(route, destination) {
    const routeParts = route.split("/").filter(Boolean);
    const destinationParts = destination.split("/").filter(Boolean);
    if (routeParts.length !== destinationParts.length)
        return false;
...
function matchRoute(destination, routes) {
...
function isRuntimeDestination(destination) {
...
function actionLabel(interaction) {
    if (!interaction.resolved)
        return `Control at ${interaction.evidence.file}:${interaction.evidence.line}`;
    const handler = interaction.handler ?? "";
    if (/\bopenDialog\b/.test(handler))
        return "Open dialog";
    if (/\bonOpenChange\b|\bsetIsOpen\b/.test(handler))
        return "Expand or collapse section";
    if (/\bonSelect\s*\(\s*version\s*\)/.test(handler))
    ...
    if (/^href$/i.test(label))
        label = `Open ${subject}`;
    else if (/^open$/i.test(label))
        label = `Open ${subject}`;
    else if (/^label$/i.test(label))
        label = `Open ${subject} details`;
    else if (/^value$/i.test(label))
...
function subjectFromEvidence(interaction) {
    const file = interaction.evidence.file
        .split("/")
        .at(-1)
        ?.replace(/\.[^.]+$/, "") ?? "Item";
    ...
    if (entity)
        return entity;
    return (parts.find((part) => !/(?:button|dialog|section|page|panel|detail|action|review|table|row)/i.test(part)) ?? "Item");
}
function externalLabel(interaction) {
    const destination = interaction.destination ?? "";
    if (/linkedin/i.test(destination) || /linkedin/i.test(interaction.label))
        return "LinkedIn profile of {person}";
    if (/github/i.test(destination) || /github/i.test(interaction.label))
        return "GitHub profile or repository";
    if (/ashby/i.test(destination) || /ashby/i.test(interaction.label))
        return "Ashby record";
    if (/mailto:/i.test(destination))
        return "Email application";
    return `External: ${actionLabel(interaction)}`;
}
function localOutcome(interaction) {
    if (interaction.staticOutcome)
        return { kind: "view-state", label: interaction.staticOutcome, proof: "source" };
    if (interaction.kind === "input")
        return { kind: "view-state", label: "Field value changes", proof: "source" };
    if (interaction.kind === "selection")
        return { kind: "view-state", label: "Selection changes", proof: "source" };
    const handler = interaction.handler ?? "";
    const successMessage = handler.match(/toast\.success\(\s*["']([^"']+)["']/)?.[1];
    if (successMessage)
        return { kind: "completed-outcome", label: successMessage, proof: "source" };
    ...
    const calls = [...handler.matchAll(/(?:await\s+|void\s+)?([A-Za-z_$][\w$]*(?:\.[A-Za-z_$][\w$]*)*)\s*\(/g)].map((match) => match[1]);
    ...
    if (!operation)
        return undefined;
    if (/^(?:preventDefault|stopPropagation)$/i.test(operation))
        return undefined;
    if (/refetch/i.test(operation))
        return { kind: "view-state", label: "Data reload requested", proof: "source" };
    ...
    if (/submit/i.test(operation))
        return { kind: "code-boundary", label: "Runs form submission handler", proof: "source" };
...
export function buildJourneyGraph(extraction) {
    const nodes = new Map();
    const edges = new Map();
    const diagnostics = [];
    ...
    for (const route of extraction.routes) {
        ...
        for (const interactionId of route.interactions) {
            ...
            if (interaction.destination) {
                ...
                const hasConcreteCandidate = candidates.some((candidate) => Boolean(matchRoute(candidate, extraction.routes)) || isExternalDestination(candidate));
                for (const destination of candidates) {
                    ...
                    if (destinationRoute) {
                        ...
                        const external = { id: id("external", label), kind: "external-exit", label };
                ...
                if (outcome) {
                    resolvedTargets.push({
                        edgeKind: outcome.kind === "view-state"
                            ? "updates"
    ...
    diagnostics.sort((left, right) => left.evidence.file.localeCompare(right.evidence.file) || left.evidence.line - right.evidence.line);
    const unresolvedRouteInstances = new Set(diagnostics
        .filter((diagnostic) => !diagnostic.interactionId.startsWith("system-transition:"))
        .map((diagnostic) => `${diagnostic.route}\u0000${diagnostic.interactionId}`)).size;
    const unresolvedSystemTransitions = new Set(diagnostics
        .filter((diagnostic) => diagnostic.interactionId.startsWith("system-transition:"))
        .map((diagnostic) => diagnostic.interactionId)).size;
    const unresolvedControlIds = new Set(diagnostics
        .filter((diagnostic) => !diagnostic.interactionId.startsWith("system-transition:"))
