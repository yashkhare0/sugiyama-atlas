...
function areaForRoute(route, labels) {
...
function nextRoute(appDirectory, filePath) {
...
function staticJsxAttribute(node, name) {
...
function componentSource(node) {
    ...
    if (Node.isJsxSelfClosingElement(element))
        tag = element.getTagNameNode();
    else if (Node.isJsxElement(element))
        tag = element.getOpeningElement().getTagNameNode();
...
function routeElementSource(node) {
    if (Node.isJsxOpeningElement(node) || Node.isJsxSelfClosingElement(node)) {
        const attribute = node.getAttribute("element");
        if (attribute && Node.isJsxAttribute(attribute))
            return componentSource(attribute.getInitializer() ?? attribute);
    }
    if (Node.isObjectLiteralExpression(node)) {
        const property = node.getProperty("element") ?? node.getProperty("Component");
        if (property && Node.isPropertyAssignment(property))
            return componentSource(property.getInitializer() ?? property);
    }
    return undefined;
}
...
export function detectFramework(config, cwd) {
    if (config.framework)
    ...
    throw new Error("Could not detect a supported frontend. Expected a Next.js app directory or a package.json containing Vite. Set framework explicitly if detection is ambiguous.");
}
export function discoverNextRoutes(project, config, cwd) {
    const labels = config.areaLabels ?? {};
    const appDirectory = path.resolve(cwd, config.appDirectory ?? path.join(config.sourceRoot, "app"));
    const sources = [];
    const rootLayout = project.getSourceFile(path.join(appDirectory, "layout.tsx"));
    if (rootLayout)
        sources.push({ area: "Global shell", file: rootLayout, kind: "layout", path: "Global shell" });
    ...
    for (const file of project
        .getSourceFiles()
        .filter((source) => source.getBaseName() === "layout.tsx" &&
...
export function discoverViteRoutes(project, config, cwd) {
    const labels = config.areaLabels ?? {};
    ...
    for (const file of project.getSourceFiles().filter((source) => source.getFilePath().startsWith(sourceRoot))) {
        for (const node of file.getDescendants().filter(Node.isJsxSelfClosingElement)) {
            if (node.getTagNameNode().getText().split(".").at(-1) !== "Route")
                continue;
    ...
    if (routes.size === 0) {
        throw new Error("Vite adapter found no statically declared React Router routes. Supported forms are <Route path=\"...\" element={<Page />} /> and object routes with literal path plus element or Component.");
    }
    return [...routes.values()].sort((left, right) => left.path.localeCompare(right.path));
}
//# sourceMappingURL=framework-adapters.js.map