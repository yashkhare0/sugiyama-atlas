...
import { defaultJourneyConfig, readJourneyConfig } from "./config.js";
import { extractJourneys } from "./extract-next.js";
import { renderJourneyHtml } from "./render-html.js";
function usage() {
...
function selectedConfig(argument) {
    if (argument)
        return readJourneyConfig(argument);
    for (const candidate of ["sugiyama.config.json", "journeys.config.json"]) {
        if (fs.existsSync(candidate))
            return readJourneyConfig(candidate);
    }
...
function run() {
    ...
    fs.writeFileSync(output.replace(/\.html$/i, ".diagnostics.json"), `${JSON.stringify(manifest.diagnostics, null, 2)}\n`);
    console.log(`Wrote ${output}`);
    console.log(`${manifest.stats.routes} screens, ${manifest.stats.actions} action instances, ${manifest.stats.unresolvedSourceControls} unique unresolved controls (${manifest.stats.unresolvedRouteInstances} route instances)`);
    console.log(`${manifest.stats.codeBoundaryInstances} callback boundaries, ${manifest.stats.deadEndInstances} no-handler dead ends, ${manifest.stats.missingLabelSourceControls} source controls without a visible label, ${manifest.stats.dynamicDestinations} runtime destinations`);
    if (manifest.diagnostics.length > 0) {
        for (const diagnostic of manifest.diagnostics.slice(0, 20)) {
            console.error(`${diagnostic.evidence.file}:${diagnostic.evidence.line}:${diagnostic.evidence.column} ${diagnostic.message}`);
        }
